...
> 每项必须同时具有 Web、iOS 原生、三语、浅深色、无障碍、权限/隐私、分析事件、测试和真实服务端；不接受静态占位或仅一端实现。支付、聊天和 Android 会扩大数据/合规/运维边界，先提交 ADR 与范围确认，再进入实现。
...
- [ ] **Step 6: 决定 Android 范围**

若“app”要求覆盖 Android，则在产品/账号/维护预算确认后创建原生 Android 对等计划；当前明确要求的 iOS 原生交付不得被跨平台壳替代。
