        ...
        ScrollViewReader { proxy in
            ScrollView {
                LazyVStack(alignment: .leading, spacing: 24) {
                    ...
                    if store.error != nil, !store.items.isEmpty {
        ...
        case .viewEvent(let reference):
            ...
            if CoreJourneyUIFixtureState.resolve() == .itinerary,
               let event = item.event {
                model.router.show(event: event.coreJourneyDetailFixture, in: .activities)
                return
            }
#endif
            model.router.push(.event(reference), in: .activities)
        case .none:
            break
        }
    }
...
private struct MyActivityNativeRow: View {
    ...
    @ViewBuilder
    private var cancellationMenu: some View {
        if let cancel {
            Menu {
    ...
    private var isProminentAction: Bool {
        ...
        case .cancelRegistration, .viewStatus, .viewEvent, .none: false
        }
    }
...
private struct MyActivityNativeCover: View {
    ...
    private var fallback: some View {
        LinearGradient(
            colors: [SpottColor.twilight.opacity(0.92), SpottColor.mint.opacity(0.76)],
            startPoint: .topLeading,
            endPoint: .bottomTrailing
        )
        .overlay {
            Image(systemName: event?.format == .online ? "video.fill" : "calendar")
