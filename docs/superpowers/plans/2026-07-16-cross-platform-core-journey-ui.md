...
先把 `maplibre-gl` 加入 Web dependencies 并更新 lockfile；测试只替换 MapLibre 的 WebGL 边界，不断言 mock marker，自身断言来自真实 `EventSummary.coordinate` 到 adapter input 的行为。
