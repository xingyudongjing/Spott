...
export function feedbackCopy(locale: Locale) {
  ...
  return {
    ...
    loading: text("feedback.loading"),
    loadError: text("feedback.loadError"),
    retry: text("feedback.retry"),
    submitError: text("feedback.submitError"),
    savedRefreshFailed: text("feedback.savedRefreshFailed"),
    unavailable: text("feedback.unavailable"),
