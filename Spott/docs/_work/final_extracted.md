<!-- source: docs/Spott-iOS-Web全栈开发文档-V1.0-20260715.docx -->

SPOTT / PRODUCT ENGINEERING

Spott iOS 原生 App+ Web 全栈开发文档

面向 10 万用户的高可用、可同步、可持续演进研发基线

SWIFT 6.3 · NEXT.JS 16.2 · NESTJS 11 · POSTGRESQL 18.4
...
借鉴 Luma 的高级感与内容优先原则，建立 Spott 独立的城市活动设计语言。
...
共 20 个主章节与 4 个附录；详细层级可在 Word 导航窗格中查看。

<!-- TABLE 3: 13 rows x 4 cols -->
| 章节 | 主题 | 章节 | 主题 |
| --- | --- | --- | --- |
| 1. | 执行摘要 | 13. | 安全、隐私与合规工程 |
| 2. | 产品需求基线与范围 | 14. | 非功能要求与 SLO |
| 3. | 视觉与体验设计方向 | 15. | 埋点、监控与数据治理 |
| 4. | 信息架构与页面规格 | 16. | 测试与质量保证 |
| 5. | 总体系统架构 | 17. | DevOps、迁移与发布 |
| 6. | Web 与 iOS 数据同步设计 | 18. | 个人开发路线与交付门禁 |
| 7. | PostgreSQL 数据架构 | 19. | 主要风险与缓解 |
| 8. | API 契约设计 | 20. | 验收与需求追踪 |
| 9. | iOS 原生开发规范 | 附录 A. | 配置中心键建议 |
| 10. | Web 与运营后台开发规范 | 附录 B. | 稳定错误码目录 |
| 11. | 后端模块详细设计 | 附录 C. | 外部参考与复核日期 |
| 12. | 核心状态机与业务规则 | 附录 D. | 最终技术原则 |
...
发现闭环：地区与兴趣推荐 → 活动详情 → 收藏/分享 → 报名或候补。

履约闭环：发布 → 审核 → 报名占位 → 通知 → 签到 → 反馈 → 再次参与。

社区闭环：一次活动 → 关联群组 → 公告与后续活动 → 长期复购关系。

## 1.2 技术结论

<!-- TABLE 4: 9 rows x 2 cols -->
| 领域 | 结论 |
| --- | --- |
...
[Caption] 图 1 · Spott 高可用总体架构

## 5.2 逻辑组件

<!-- TABLE 13: 11 rows x 3 cols -->
| 组件 | 职责 | 扩展策略 |
| --- | --- | --- |
...
[Caption] 图 2 · Web 与 iOS 可靠增量同步

## 6.2 权威 ID 与版本字段

所有可同步实体使用服务端生成 UUIDv7；公开 URL 使用独立、可轮换的 public_slug，不得暴露自增主键。核心字段：

id uuid primary key,version bigint not null default 1,created_at timestamptz not null default now(),updated_at timestamptz not null default now(),deleted_at timestamptz null,created_by uuid null,updated_by uuid null

每次业务变更在同一 PostgreSQL 事务中更新实体、写审计信息、写 change_log 和 outbox_events。version 只递增不回退；软删除以 Tombstone 同步，客户端收到后清除可见数据并保留最小同步标记。

## 6.3 客户端同步状态

<!-- TABLE 17: 8 rows x 2 cols -->
| 字段 | 说明 |
| --- | --- |
...
启动：读取本地数据库并立即渲染安全缓存；验证会话；调用 /v1/sync/pull?cursor=...&limit=500。

应用：在本地单事务中按序应用 Upsert/Tombstone；只有全部成功才推进 cursor。

推送：按创建依赖顺序提交离线操作；请求携带 Idempotency-Key、operationId、baseVersion 与 deviceId。

实时：前台建立 WebSocket；消息只携带变更序列和主题，收到后触发增量 Pull，不把 WebSocket 消息本身当完整事实。

后台：APNs 静默通知或 BGAppRefresh 仅负责唤醒；系统不保证执行时仍可在下次前台追赶。

重连：以已提交 cursor 继续，允许服务端返回 cursor_expired 并要求从受控快照重新同步。

## 6.5 同步 API 示例

GET /v1/sync/pull?cursor=824018&limit=500{ "nextCursor": 824097, "hasMore": false, "serverTime": "2026-07-15T08:20:31.402Z", "changes": [ { "seq": 824019, "entityType": "registration", "entityId": "019b...", "operation": "upsert", "version": 7, "changedFields": ["status", "confirmedAt"], "payload": {"status": "confirmed", "availableActions": ["cancel", "viewTicket"]} } ]}

POST /v1/sync/pushIdempotency-Key: 58f0d2c0-...{ "deviceId": "019a...", "operations": [ { "operationId": "58f0...", "entityType": "profile", "entityId": "019b...", "action": "patch", "baseVersion": 12, "patch": {"bio": "东京周末徒步与城市摄影"} } ]}

## 6.6 冲突解决矩阵

<!-- TABLE 18: 10 rows x 3 cols -->
| 数据类型 | 策略 | 客户端行为 |
| --- | --- | --- |
...
[Caption] 图 3 · PostgreSQL 领域数据边界

## 7.2 核心表目录

### 7.2.1 身份与资料

<!-- TABLE 20: 10 rows x 3 cols -->
| 表 | 核心字段 | 关键约束 |
| --- | --- | --- |
...
create unique index uq_active_registrationon events.registrations(event_id, user_id)where status in ('pending', 'confirmed', 'waitlisted', 'checked_in');create unique index uq_active_phoneon identity.phone_bindings(phone_hash)where unbound_at is null;create index ix_event_discovery_geoon events.event_locations using gist(point);create index ix_event_title_trgmon events.events using gin(title gin_trgm_ops);create index ix_change_log_user_seqon sync.change_log(user_scope, seq);
...
{ "error": { "code": "REGISTRATION_CAPACITY_FULL", "message": "活动名额刚刚报满，可以加入候补。", "requestId": "req_019b...", "retryable": false, "fieldErrors": [], "actions": [{"type": "joinWaitlist", "label": "加入候补"}], "meta": {"waitlistEnabled": true} }}

<!-- TABLE 28: 9 rows x 3 cols -->
| HTTP | 使用 | 示例代码 |
| --- | --- | --- |
...
SpottApp/ App/ # App entry、路由、依赖组装、生命周期 DesignSystem/ # Tokens、Typography、Components、Motion、A11y Core/ API/ # OpenAPI 生成层、Auth、Retry、Error Mapping Persistence/ # SwiftData 模型、Migration、Repositories Sync/ # SyncEngine、OperationQueue、Cursor、Conflict Security/ # Keychain、App Attest、敏感缓存策略 Analytics/ # 事件协议、Consent、Trace correlation Features/ Discovery/ EventDetail/ Registration/ EventComposer/ HostStudio/ Groups/ Wallet/ Notifications/ Safety/ Profile/ Integrations/ # StoreKit、APNs、MapKit、EventKit、Photos、ActivityKit Resources/ # Localizable、Assets、PrivacyInfo Tests/
...
actor SyncEngine { func bootstrap(userID: UserID) async throws func pull(reason: SyncReason) async throws -> SyncResult func enqueue(_ operation: PendingOperation) async throws func flushPendingOperations() async -> FlushResult func handleRealtimeHint(sequence: Int64) async func resetSensitiveScope(reason: ResetReason) async throws}
...
apps/ web/ # 公开站与用户/局头应用 ops/ # 运营后台，独立安全边界packages/ design-tokens/ # iOS/Web 共享语义令牌源 ui/ # Web 组件库 api-client/ # OpenAPI 生成类型与调用层 domain/ # 纯 TypeScript 枚举/格式化/错误映射 analytics/ # 埋点 Schema 与 Consent
...
[Caption] 图 5 · 报名与签到状态机

<!-- TABLE 34: 9 rows x 3 cols -->
| 当前状态 | 允许进入 | 积分/容量影响 |
| --- | --- | --- |
...
Pull Request → static checks / unit tests → API contract compatibility → PostgreSQL migration up/down safety audit → integration + security scan → preview environment / iOS test build → e2e + visual + accessibility → approval → staged production deploy → smoke + SLO watch → gradual feature enablement
...
访客从分享链接进入 Web 活动详情，无登录即可查看公开信息；报名时登录并验证日本手机号，iOS 登录同账号后看到已报名状态。

iOS 创建活动草稿，Web 工作台继续编辑并提交；iOS 收到审核状态更新，草稿字段与版本一致。

最后一个名额同时收到多端报名，仅一个确认，其余获得候补选择；钱包无重复扣分。

局头取消活动，公开页快速变为取消，所有报名/候补收到站内记录，报名积分按规则退回，活动费提示联系局头。

动态二维码签到后 Web 工作台实时看到到场，用户获得一次到场奖励和评价资格；重复扫码不重复发放。

群组满 50 人后新增加入失败但现有成员正常；积分扩容成功后允许新增，扩容交易与审计一致。

运营下架高风险活动，公开页、分享、报名、iOS 缓存均收敛为不可用；精确地址从未授权响应与缓存移除。

Apple 商店重复交易回调只发放一次；退款生成冲正，负付费积分限制只影响配置的高风险能力。

PostgreSQL 恢复演练后账本、报名、签到、Outbox 和同步游标一致；RPO/RTO 达标。

所有 P0 核心页面通过中文/日文长文本、深色模式、Dynamic Type、VoiceOver/键盘与弱网测试。
...
PostgreSQL 保存事实，客户端缓存保存体验；二者不可颠倒。

WebSocket 提供及时性，增量 Pull 提供可靠性；二者不可替代。

关键状态由服务端状态机决定，客户端只展示可行动作。

幂等键防重复，数据库约束守底线，审计日志解释发生了什么。

活动费与平台积分在产品、代码、账本和文案上彻底隔离。

视觉借鉴高级感与内容优先，不借用第三方身份；Spott 必须一眼有自己的城市气质。

所有商业化能力先可配置、可灰度、可回滚，再考虑放大。

如果同步、积分、安全或隐私无法被测试证明，就不能视为完成。
